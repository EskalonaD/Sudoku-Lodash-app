...
var _sudokuPresenter = __webpack_require__(335);

var _data = __webpack_require__(338);

(0, _sudokuPresenter.startGame)(_data.startingPositions);
...
exports.startGame = exports.renderWinView = exports.renderTimer = undefined;

var _lodash = __webpack_require__(130);

var _lodash2 = _interopRequireDefault(_lodash);

var _sudokuModel = __webpack_require__(337);
...
var selectCell = function selectCell(cellNode) {
...
var deselectCell = function deselectCell(cellNode) {
    cellNode.classList.remove('selected');
    cellNode.classList.remove('js-selected');
};

var addClickEvent = function addClickEvent(cell) {
    var onBlur = function onBlur() {
        return deselectCell(cell);
    };

    var escapeHandling = function escapeHandling(event) {
...
var objectMapperPredefinedValues = function objectMapperPredefinedValues(data) {
    return _lodash2.default.reduce(data, function (acc, cellArr) {
        acc['' + cellArr[0] + cellArr[1]] = cellArr[2];
        return acc;
    }, {});
};

var nodeExist = function nodeExist(className) {
    return _lodash2.default.some(document.getElementsByClassName(className));
};

var renderTimer = exports.renderTimer = function renderTimer() {
    var t = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 0;
    var table = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : tableModel;

    document.getElementsByClassName('js-timer')[0].innerHTML = _lodash2.default.template('<span class="js-spent-time"><%= t %></span> seconds')({ t: t });
};

var renderWinView = exports.renderWinView = function renderWinView(buttonEventHandler) {
    var winView = document.getElementsByClassName('js-win-view-template')[0].innerHTML;
    var time = document.getElementsByClassName('js-spent-time')[0].textContent;
    document.getElementsByClassName('js-win-view-wrapper')[0].innerHTML = _lodash2.default.template(winView)({ t: time });
    document.getElementsByClassName('js-again-button')[0].addEventListener('click', buttonEventHandler);
};

var winEvent = new Event('win');

var addTimerToWin = function addTimerToWin(renderTimer) {
    var time = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 0;
    var table = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : tableModel;

    renderTimer(time++);

    if (!(0, _sudokuModel.validateWin)(table, nodeExist('js-invalid'))) _lodash2.default.delay(function () {
        return addTimerToWin(renderTimer, time);
    }, 1000);else document.getElementsByClassName('js-table')[0].dispatchEvent(winEvent);
};

var setInitialValues = function setInitialValues(table, data) {
    var cellNodes = document.getElementsByClassName('js-sudoku-cell');

    _lodash2.default.each(cellNodes, function (cell) {
        return !isNaN(+cell.textContent) && cell.classList.add('js-predefine-cell');
    });
};

var restartGame = function restartGame(newData) {
...
var startGame = exports.startGame = function startGame(data) {
    var table = (0, _sudokuModel.createTable)();
    var random = Math.trunc(Math.random() * data.length);
    var startedValues = data[random];
    var newData = _lodash2.default.filter(data, function (value, propName) {
        return propName !== random;
    });
    var restartFunction = function restartFunction(data) {
        return function () {
            return restartGame(data);
        };
    };

    renderTable = _lodash2.default.partial(function (data, table) {
        var isValid = (0, _sudokuModel.validateTable)(table);
        var domTable = document.getElementsByClassName('js-table')[0];
        var cellNodes = document.getElementsByClassName('js-sudoku-cell');
        var predefinedValuesObject = objectMapperPredefinedValues(data);

        domTable.innerHTML = tableTemplate({ table: table, isValid: isValid });
        _lodash2.default.each(cellNodes, function (cell) {
    ...
    (0, _sudokuModel.setPredefineValues)(table, startedValues);
    renderTable(table);
    setInitialValues(table, startedValues);
    addTimerToWin(renderTimer);
    addKeyEvent();

    document.getElementsByClassName('js-table')[0].addEventListener('win', function () {
        return renderWinView(restartFunction(newData));
    });
};
...
exports.setPredefineValues = exports.validateWin = exports.setCell = exports.validateTable = exports.createTable = undefined;
...
var validateWin = exports.validateWin = function validateWin(table, isInvalid) {
    return !_lodash2.default.some(_lodash2.default.flatMap(table), function (value) {
        return typeof value !== 'number';
    }) && !isInvalid;
};

var setPredefineValues = exports.setPredefineValues = function setPredefineValues(table, dataArr) {
    _lodash2.default.each(dataArr, function (cellArr) {
 ...
 * */
var startingPositions = exports.startingPositions = [[[0, 0, 9], [0, 1, 2], [0, 2, 7], [0, 7, 6], [0, 8, 3], [1, 0, 3], [1, 3, 8], [1, 5, 9], [2, 2, 5], [3, 0, 1], [3, 1, 8], [3, 3, 7], [5, 5, 3], [5, 7, 8], [5, 8, 7], [6, 6, 1], [7, 3, 3], [7, 5, 1], [7, 8, 6], [8, 0, 5], [8, 1, 1], [8, 6, 7], [8, 7, 9], [8, 8, 4]], [[0, 2, 2], [0, 6, 4], [1, 3, 8], [2, 0, 9], [2, 2, 1], [2, 4, 4], [2, 6, 5], [3, 0, 6], [3, 2, 8], [3, 3, 9], [3, 4, 5], [3, 5, 3], [4, 4, 7], [5, 3, 4], [5, 4, 2], [5, 5, 6], [5, 6, 9], [5, 8, 3], [6, 2, 6], [6, 4, 1], [6, 6, 8], [6, 8, 5], [7, 5, 5], [8, 2, 5], [8, 6, 3]], [[0, 3, 9], [0, 6, 8], [0, 8, 5], [1, 0, 9], [1, 5, 3], [1, 8, 4], [2, 1, 7], [2, 3, 6], [2, 5, 4], [2, 8, 1], [3, 4, 3], [3, 8, 9], [5, 0, 3], [5, 4, 5], [6, 0, 6], [6, 3, 8], [6, 5, 1], [6, 7, 7], [7, 0, 2], [7, 3, 3], [7, 8, 8], [8, 0, 1], [8, 2, 5], [8, 5, 2]]];
